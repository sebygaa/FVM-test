...
T_feed = float(sys.argv[1])
P_feed = float(sys.argv[2])
f_nam1 = f'PDE_C_data_T{T_feed:03.0f}K_P{P_feed:02.0f}bar.csv'
f_nam2 = f'PDE_q_data_T{T_feed:03.0f}K_P{P_feed:02.0f}bar.csv'
#print(T_feed, P_feed, f_nam)
#print('[ P_feed ]')
#print(f' {P_feed:02.0f}')
...
Column_head_t = ['time (s)']
Column_head_C = []
Column_head_q = []
for ii in range(N):
    Column_head_C.append('C1 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/m^3)')
    Column_head_q.append('q1 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/kg)')
for ii in range(N):
    Column_head_C.append('C2 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/m^3)')
    Column_head_q.append('q2 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/kg)')
for ii in range(N):
    Column_head_C.append('C3 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/m^3)')
    Column_head_q.append('q3 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/kg)')
for ii in range(N):
    Column_head_C.append('C4 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/m^3)')
    Column_head_q.append('q4 (z='+ f'{acc1.z[ii]:.3f})' +' (mol/kg)')

di_csv1 = {'time (s)': t_ran}
di_csv2 = {'time (s)': t_ran}

for ii,cc in enumerate(Column_head_C):
    C_tmp = acc1.y_res[:,ii]
    di_csv1[cc] = C_tmp

for ii,cc in enumerate(Column_head_q):
    q_tmp = acc1.y_res[:,4*N + ii]
    di_csv2[cc] = q_tmp

df_csv1 = pd.DataFrame(di_csv1,)
df_csv2 = pd.DataFrame(di_csv2,)
#df_csv.style.hide_index()
df_csv1.set_index('time (s)')
df_csv1.to_csv(f_nam1)

df_csv2.set_index('time (s)')
df_csv2.to_csv(f_nam2)
...
acc1.graph_timelapse_y(3,t_frame=1000,
                       label='CO$_{2}$ mole frac.',
                       filename =dir_name+f_prefix+'y3_timelapse.gif',
                       y_limits=[-0.04,1.04], interval = 100)
...
acc1.feed_condi_y(y_feed_list, 1, T_feed2, u_feed2, 
                  Cv_in= 0.02, const_u = True,)
# Outlet condition
P_end2 = 1.1    # P_end1 = 5.5 (bar)
Cv_out2 = 0.08  # Cv_out1 = 0.02
acc1.outlet_condi_y(y_end_list, P_end2,
                    T_end, Cv_out2)
